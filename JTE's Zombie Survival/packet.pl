...
sub send_rprawchat() {
	my ($id,$distm,$msg) = @_;
	my (@p1,@p2,@pinc,@d1,@d2,$nick0,$nick1);
	my ($err1,$err2,$i,$l,$m,$n,$dis) = 0;
	foreach (@{$server{'users'}}) {
		next unless defined($_);
		$nick0 = lc($_->{'account'});
		$nick1 = $_->{'hearall'};
		if (!defined($_->{'hearall'})) { $nick1 = 0 };
		if ($_->{'active'} && (($server{'admin'}{$nick0} < 100) || ($nick1 < 1))) {
		$dis = 0;
		$p1[0] = floor($_->{'pos'}[0] / 32);
		$p2[0] = floor($server{'users'}[$id]{'pos'}[0] / 32);
		$p1[1] = floor($_->{'pos'}[1] / 32);
		$p2[1] = floor($server{'users'}[$id]{'pos'}[1] / 32);
		$p1[2] = floor($_->{'pos'}[2] / 32);
		$p2[2] = floor($server{'users'}[$id]{'pos'}[2] / 32);
		$d1[0] = $p2[0] - $p1[0];
		$d1[1] = $p2[1] - $p1[1];
		$d1[2] = $p2[2] - $p1[2];
		if($d1[0] < 0) { $pinc[0] = -32; } else { $pinc[0] = 1; }
		if($d1[1] < 0) { $pinc[1] = -32; } else { $pinc[1] = 1; }
		if($d1[2] < 0) { $pinc[2] = -32; } else { $pinc[2] = 1; }
		$l = abs($d1[0]);
		$m = abs($d1[1]);
		$n = abs($d1[2]);
		$d2[0] = 2^$d1[0];
		...
		if (($l >= $m) && ($l >= $n))
		{
			...
			$err2 = $d2[2] - $l;
			for($i = 1; $i < $l+1; $i++) {
				$dis = $dis + &get_tileinfo(&map_getblock($p1[0],$p1[1],$p1[2]))->{'rpd'};
				if($err1 > 0) {
					$p1[1] = $p1[1]+$pinc[1];
					$err1 = $err1 - $d2[0];
				}
		...
		elsif (($m >= $l) && ($m >= $n))
		{
			...
			$err2 = $d2[2] - $m;
			for($i = 1; $i < $m+1; $i++) {
				$dis = $dis + &get_tileinfo(&map_getblock($p1[0],$p1[1],$p1[2]))->{'rpd'};				if($err1 > 0) {
					$p1[0] = $p1[0]+$pinc[0];
					$err1 = $err1 - $d2[1];
				}
				if($err2 > 0) {
		...
		if ($dis < $distm) { &send_raw($_->{'sock'},13,$id,$msg); }
		} else { &send_raw($_->{'sock'},13,$id,$msg); }
	}
}
...
sub pl2zombie() {
	...
	my $znick;
	my $onick;
	$onick = $server{'users'}[$id]{'nick'};
	$dnick = "&4" . $server{'users'}[$id]{'nick'};
	$znick = "&a" . $server{'users'}[$id]{'nick'};
	$server{'users'}[$id]{'nick'} = $dnick;
	if ($dnick eq $server{'users'}[$id]{'account'}) { undef $server{'nick'}{lc($server{'users'}[$id]{'account'})}; }
	else { $server{'nick'}{lc($server{'users'}[$id]{'account'})} = $dnick; }
	unless ($server{'users'}[$id]{'hide'}) {
		&global_die($id);
		&global_spawn($id,$dnick,@{$server{'users'}[$id]{'pos'}},@{$server{'users'}[$id]{'rot'}});
	}
	$server{'users'}[$id]{'iszomb'} = 1;
	&send_rawchat($id,"$znick is now a zombie!");
}
...
sub handle_build() {
	...
	if(defined($server{'users'}[$id]{'iszomb'}) && $type != 0) { &send_blockchange($id,$pos_x,$pos_y,$pos_z,&map_getblock($pos_x,$pos_y,$pos_z)); }
	elsif ($server{'users'}[$id]{'tree'} && $type == 17) {
	...
	elsif ($type == 44 && &map_getblock($pos_x,$pos_y-1,$pos_z) == 44) {
		&map_setblock($pos_x,$pos_y,$pos_z,0);
		&send_blockchange($id,$pos_x,$pos_y,$pos_z,&map_getblock($pos_x,$pos_y,$pos_z));
		&map_setblock($pos_x,$pos_y-1,$pos_z,43);
		&send_blockchange($id,$pos_x,$pos_y-1,$pos_z,&map_getblock($pos_x,$pos_y-1,$pos_z));
	}
	else {
...
sub handle_blockchange() {
	...
	if ($type > 49) {
		print "WARNING: Unknown block type $type selected by ".$server{'users'}[$id]{'nick'}."; Changed to stone.\n";
		$type = 1;
	}
	...
	if ($action == 0) {
		...
		if ($server{'users'}[$id]{'paint'}) {
			...
			elsif(defined($server{'users'}[$id]{'iszomb'})) { &send_blockchange($id,$pos_x,$pos_y,$pos_z,$display); }
...
sub handle_chat() {
	...
	if ($msg =~ m|^/(\S+)\s*(.*)|) {
		...
		$msg = '&2//'.$server{'users'}[$id]{'nick'}.'&2 (OOC):&f '.$1;
		foreach (@{$server{'users'}}) {
			next unless defined($_);
			&send_raw($_->{'sock'},13,$id,$msg) if ($_->{'active'});
		}
	...
	else { &send_rpchat($id,8.5,$msg); } # Normal map chat
}
