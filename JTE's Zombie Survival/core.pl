...
sub update_position() {
	...
	if ($changed > 0 && defined($user->{'iszomb'}) && $user->{'iszomb'} > 0) {
	foreach (@{$server{'users'}}) {
		next unless defined($_) && $_->{'active'};
		next if ($user == $_);
		next if (defined($_->{'iszomb'}) && ($_->{'iszomb'} > 0));
		@pos2 = @{$_->{'pos'}};
		if($pos[0] > $pos2[0]) { $dp[0] = $pos[0] - $pos2[0] } else { $dp[0] = $pos2[0] - $pos[0] }
		if($pos[1] > $pos2[1]) { $dp[1] = $pos[1] - $pos2[1] } else { $dp[1] = $pos2[1] - $pos[1] }
		if($pos[2] > $pos2[2]) { $dp[2] = $pos[2] - $pos2[2] } else { $dp[2] = $pos2[2] - $pos[2] }
		$dp2 = $dp[0] + $dp[1] + $dp[2];
		next if $dp2 > 31;
		$_->{'iszomb'} = 1;
		&pl2zombie($_->{'id'});
