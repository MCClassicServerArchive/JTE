...
$server{'commands'} = {
	HELP => [\&cmd_help,0],
	RULES => [\&cmd_rules,0,'The rules. VERY IMPORTANT!'],
	...
	TOZOMBIE => [\&cmd_tozombie,100],
	RESETZ => [\&cmd_resetzomb,100],
	HEARALL => [\&cmd_hearall,100],
};
...
sub cmd_resetzomb() {
	...
	my $nick;
	my $id2;
	&send_rawchat($id,"&eResetting the game...");
	foreach (@{$server{'users'}}) {
		...
		$_->{'nick'} = $_->{'account'};
		$id2 = $_->{'id'};
		unless ($server{'users'}[$id2]{'hide'}) {
			&global_die($id2);
			&global_spawn($id2,$_->{'nick'},@{$server{'users'}[$id2]{'pos'}},@{$server{'users'}[$id2]{'rot'}});
		}
	}
	&send_rawchat($id,"&aNobody is a zombie anymore!");
}

sub cmd_hearall() {
	my $id=shift;
	my $nick1 = $server{'users'}[$id]{'hearall'};
	if (!defined($server{'users'}[$id]{'hearall'})) { $nick1 = 0; }
	if ($server{'users'}[$id]{'hearall'} > 0)
	{
		&send_msg($id,"&eYou cannot hear all anymore.");
		$server{'users'}[$id]{'hearall'} = 0;
	} else {
		&send_msg($id,"&eYou can hear all now.");
		$server{'users'}[$id]{'hearall'} = 0;
	}
}
sub cmd_tozombie() {
	...
	foreach (@{$server{'users'}}) {
		next unless defined($_);
		next unless lc($_->{'account'}) eq $friend;
		$id2 = $_->{'id'};
		&pl2zombie($id2);
	}
}
...
sub cmd_rules() {
	my $id = shift;
	&send_msg($id,"&d-= RULES: =-");
	&send_msg($id,"&e1. &fZombies can dig but not build.");
	&send_msg($id,"&e2. &fZombies try to infect humans by touching them.");
	&send_msg($id,"&e3. &cDO NOT BUILD HOVERHOUSES OR BE BANNED.");
	&send_msg($id,"&e4. &fUse OOC (#) only for discussion about the game!");
	&send_msg($id,"&e5. &fWithout OOC, only people near you can hear you.");
	&send_msg($id,"&e6. &fDo not use hacks.");
	&send_msg($id,"&e7. &fHave lots of fun! :)");
}
sub cmd_help() {
	my $id = shift;
...
sub cmd_build() {
	...
	unless (@_) {
		unless ($server{'users'}[$id]{'build'}) {
			...
			if ($server{'users'}[$id]{'admin'} >= 20) {
				&send_msg($id,'solid/admin, grass, gold, iron, coal, lava sponge,');
				&send_msg($id,'dynamite, super sponge, watervator, doublestep');
			}
			elsif ($server{'users'}[$id]{'admin'} >= 200) {
				&send_msg($id,'solid/admin, grass, water, lava, gold, iron, coal,');
				&send_msg($id,'lava sponge, dynamite, super sponge, watervator, doublestep');
			}
			else { &send_msg($id,'grass, gold, iron, coal, lava sponge, super sponge, watervator,');
			       &send_msg($id,'doublestep'); }
	...
	elsif ($type =~ /doublestep/) { $type = 43; $typename = 'doublestep'; }
